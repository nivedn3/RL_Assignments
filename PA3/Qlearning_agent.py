...
class q_agent(Agent):
	...
	def agent_step(self,Reward,Obs):
		...
		maxa = max(self.thetax[x1][0] + self.thetay[y1][0] + self.thetaxy[x1][y1][0],self.thetax[x1][1] + self.thetay[y1][1] + self.thetaxy[x1][y1][1],self.thetax[x1][2] + self.thetay[y1][2] + self.thetaxy[x1][y1][2],self.thetax[x1][3] + self.thetay[y1][3] + self.thetaxy[x1][y1][3] )
		delta = delta  + self.gamma*maxa

		alpha = 0.1
		self.thetax[x][last_action] = self.thetax[x][last_action] +  alpha*delta
		self.thetay[y][last_action] = self.thetay[y][last_action] +  alpha*delta
		self.thetaxy[x][y][last_action] = self.thetaxy[x][y][last_action] + alpha*delta
		...
		if random.random() < self.epsilon:
			...
			#print k
		#print Reward,self.thetax,self.thetay,self.thetaxy
	...
	def agent_end(self,Reward):
		...
		alpha = 0.1
		self.thetax[x][last_action] = self.thetax[x][last_action] +  alpha*delta
		self.thetax[y][last_action] = self.thetax[y][last_action] +  alpha*delta
		self.thetaxy[x][y][last_action] =  self.thetaxy[x][y][last_action] + alpha*delta
